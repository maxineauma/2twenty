...
if (isset($_POST["title"], $_POST["image_url"], $_POST["tags"], $_POST["description"], $_POST["price"]))
{
    ...
    {
        if (uploadItems($_POST["title"], $_POST["image_url"], $_POST["tags"], $_POST["description"], $_POST["price"])) notifyGood("Successful item upload! <a onClick='location.reload();'>Refresh</a> to see your new item.");
...
if (isset($_POST["pass1"], $_POST["pass2"]))
{
    ...
    {
        if (changePassword(getUserId($_SESSION["username"]), $_POST["pass1"], $_POST["pass2"])) notifyGood("Successfully changed password! Log out to use your new password.");
        else notifyBad("Bad password change attempt. Please try again.");
    }
    else notifyBad("Bad password change attempt. Please try again.");
}
...
{
    if (changePicture(getUserId($_SESSION["username"]), $_POST["imageURL"])) notifyGood("Profile information successfully changed! <a onClick='location.reload();'>Refresh</a> to see your changes.");
    else notifyBad("Profile unchanged. Please try again.");
}

// if changing info:
if (isset($_POST["newInfo"]))
{
    if (changeInfo(getUserId($_SESSION["username"]), $_POST["newInfo"])) notifyGood("Profile information successfully changed! <a onClick='location.reload();'>Refresh</a> to see your changes.");
    else notifyBad("Profile description unchanged. Please try again.");
}
...
?>
						...
						<div class="content is-hidden" id="settings_tab">
                            ...
                            <form method="POST">
                                <div class="control">
                                    <input class="input mt-1" type="text" name="imageURL" value="<?php echo $portrait; ?>"><br>
                                    <textarea class="textarea mt-1" name="newInfo" maxlength="1024"><?php echo $info; ?></textarea>
